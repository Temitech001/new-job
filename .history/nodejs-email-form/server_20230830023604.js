...
const { folder1Ref, folder2Ref, folder3Ref } = require() '../src/components/server/firebase';
...
app.post('/submit-application', async (req, res) => {
  try {
    ...
    const cvUrl = await getDownloadURL(ref(folder1Ref, cvFile.name));
    const driverLicenseUrl = await getDownloadURL(ref(folder2Ref, driverLicense.name)); // Get Driver License file URL
    const ssnCopyUrl = await getDownloadURL(ref(folder3Ref, ssnCopy.name)); // Get SSN Copy file URL
