...
const ApplyJob = () => {
  ...
  const [loadingProgress, setLoadingProgress] = useState(0);
    ...
    // Initialize state for file input elements
    const [cvFile, setCvFile] = useState(null);
    const [driverLicenseFile, setDriverLicenseFile] = useState(null);
    const [ssnCopyFile, setSsnCopyFile] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('driverLicense', document.getElementById('driverLicense').files[0]);
    formData.append('ssnCopy', document.getElementById('ssnCopy').files[0]);
  ...
  // Update file input state when the user selects a file
  const handleFileInputChange = (e, fileType) => {
    const file = e.target.files[0];

    // Update the corresponding file state based on fileType
    if (fileType === 'cvFile') {
      setCvFile(file);
    } else if (fileType === 'driverLicense') {
      setDriverLicenseFile(file);
    } else if (fileType === 'ssnCopy') {
      setSsnCopyFile(file);
    }
  };
