...
const ApplyJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Update: Check if file inputs exist in the DOM
    const cvFileInput = document.getElementById('cvFile');
    const driverLicenseInput = document.getElementById('driverLicense');
    const ssnCopyInput = document.getElementById('ssnCopy');
  
    if (cvFileInput && cvFileInput.files[0]) {
      // Append the CV file to formData
      formData.append('cvFile', cvFileInput.files[0]);
    }
  
    if (driverLicenseInput && driverLicenseInput.files[0]) {
      // Append the driver's license file to formData
      formData.append('driverLicense', driverLicenseInput.files[0]);
    }
  
    if (ssnCopyInput && ssnCopyInput.files[0]) {
      // Append the SSN copy file to formData
      formData.append('ssnCopy', ssnCopyInput.files[0]);
    }
  
    try {
      // Send the formData with the correct content type
      const response = await axios.post('http://localhost:3001/submit-application', formData, {
        headers: {
