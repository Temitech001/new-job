...
const ApplyJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append form field values to formDataToSubmit
    formDataToSubmit.append('fullName', formData.fullName);
    formDataToSubmit.append('email', formData.email);
    formDataToSubmit.append('phoneNumber', formData.phoneNumber);
    formDataToSubmit.append('coverLetter', formData.coverLetter);
  
    // Append uploaded files to formDataToSubmit
    if (formData.cvFile) {
      // Check if cvFile exists in formData before appending
      formDataToSubmit.append('cvFile', formData.cvFile);
    }
    if (formData.driverLicense) {
      // Check if driverLicense exists in formData before appending
      formDataToSubmit.append('driverLicense', formData.driverLicense);
    }
    if (formData.ssnCopy) {
      // Check if ssnCopy exists in formData before appending
      formDataToSubmit.append('ssnCopy', formData.ssnCopy);
    }
  
    // Debug: Log the values of formData
    console.log('formData:', formData);

    // Debug: Log the formDataToSubmit before sending the request
console.log('formDataToSubmit:', formDataToSubmit);
  
    try {
      // Send the formDataToSubmit with the correct content type
