...
const ApplyJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await uploadFileToFolder(cvFile, folder1Ref, submissionId);
  
    // Upload driver license to folder2
    await uploadFileToFolder(driverLicense, folder2Ref, submissionId);
  
    // Upload SSN copy to folder3
    await uploadFileToFolder(ssnCopy, folder3Ref, submissionId);
  
    // Once all files are uploaded, you can proceed with your form submission
    const formDataToSubmit = {
      fullName,
      email,
      phoneNumber,
      coverLetter,
      cvFile: cvFile.name, // Store only the file name
      driverLicense: driverLicense.name, // Store only the file name
      ssnCopy: ssnCopy.name, // Store only the file name
    };
