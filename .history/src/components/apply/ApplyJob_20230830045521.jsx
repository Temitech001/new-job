...
const ApplyJob = () => {
  ...
  await uploadFileToFolder(ssnCopy, folder3Ref);
   ...
   // Download the file content from the URL using Fetch
  const cvFileResponse = await fetch(cvFileBlob);
  const driverLicenseResponse = await fetch(driverLicenseBlob);
  const ssnCopyResponse = await fetch(ssnCopyBlob);

  let cvFileData, driverLicenseData, ssnCopyData;

  // Check if the responses are successful
  if (cvFileResponse.ok && driverLicenseResponse.ok && ssnCopyResponse.ok) {
    // Get the file content as an ArrayBuffer
    const cvFileData = await cvFileResponse.arrayBuffer();
    const driverLicenseData = await driverLicenseResponse.arrayBuffer();
    const ssnCopyData = await ssnCopyResponse.arrayBuffer();

    // return { cvFileData, driverLicenseData, ssnCopyData };


    // Now you have the file content as ArrayBuffer objects
    // You can proceed with your form submission or further processing
  } else {
    // Handle the case where one or more files failed to download
    console.error('Error downloading file(s)');
  }
    ...
    const formDataToSubmit = {
      ...
      phoneNumber,
      coverLetter,
      cvFile: { name: cvFile.name, content: cvFileData }, // Include file content
      driverLicense: { name: driverLicense.name, content: driverLicenseData }, // Include file content
      ssnCopy: { name: ssnCopy.name, content: ssnCopyData }, // Include file content
    };
  
    try {
      const response = await axios.post('http://localhost:3001/submit-application', formDataToSubmit, {
        headers: {
