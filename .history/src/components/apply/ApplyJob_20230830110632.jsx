...
const ApplyJob = () => {
  ...
  //   const { cvFileBlob, driverLicenseBlob, ssnCopyBlob } = await fetchFiles();
    ...
    const formDataToSubmit = {
      ...
      phoneNumber,
      coverLetter,
      // cvFile: { name: cvFile.name, content:  }, // Include file content
      // driverLicense: { name: driverLicense.name, content:  }, // Include file content
      // ssnCopy: { name: ssnCopy.name, content:  }, // Include file content
    };
  
    try {
      await axios.post('https://api.smtpexpress.com/send'{
        header: {
          'Authorization: Bearer <PROJECT_SECRET>'
        },
        data '{
          "subject": "My First Email from SMTPExpress",
            "sender": {
              "name": "SMTPExpress",
              "email": "hello@smtpexpress.com"
            },
          "recipients": ["you@yourmail.com"],
          "message": "Welcome to the future of Email Delivery"
      }'
      });
