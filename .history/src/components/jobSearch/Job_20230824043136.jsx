...
const Job = ({ job }) => {
  ...
  const descriptionItems = job.description;
  const formatDescription = (descriptionItems) => {
    // Split the description into sentences using periods as the delimiter
    const sentences = descriptionItems.split('. ');

    // Join all sentences except the last one back together with periods
    const formattedDescription = sentences.slice(0, -1).join('. ');
