...
const Job = ({ job, onDelete }) => {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg overflow-hidden border-solid border-2 p-2 relative">
      ...
      {isUserAuthorized && (
        < div className="bottom-2 flex justify-between absolute">
        <button
          onClick={onDelete}
          className="bg-red-500 hover:bg-red-600 text-white px-2 md:px-4 md:py-2 rounded"
        >
          Delete
        </button>
        <button
          onClick={() => handleApplyToJobClick(job.id)}{handleEditJobClick}
          className="bg-green-500 hover:bg-green-600 text-white px-2 md:px-4 md:py-2 rounded ml-4"
        >
          Edit
        </button>
        </div>
